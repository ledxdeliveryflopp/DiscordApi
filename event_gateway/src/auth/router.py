...
async def router_handle_auth_websocket(client_id: str, client_ip: str, client_type: str, websocket: WebSocket,
                                       service: AuthEventsService = Depends(init_auth_events_service)):
    return await service.handle_auth_websocket(websocket, client_id, client_ip, client_type)
