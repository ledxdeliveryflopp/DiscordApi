...
class AuthorizationRepository(BaseService):
    ...
    @staticmethod
    async def _repository_get_auth_qr_code() -> dict:
        """Получить url к qr коду авторизации"""
        qr_url = await s3_service.get_url_to_qr_file_in_s3("qr-code.png")
        return {"detail": qr_url}

    async def _repository_login_by_qr_code(self, auth_token: str) -> dict | HTTPException:
        """Авторизация через qr код"""
        token_payload = await get_token_payload(auth_token)
        user_email = token_payload.get("user_email")
        check_user = await self._find_user_by_email(user_email)
        if not check_user:
            raise UserDontExistException
        token_dict = await create_token(check_user.id, check_user.email)
        token = token_dict.get("token")
